...
// <SearchedNewsSec topic={pid} />

const SearchedNewsSec = ({ topic }: Props) => {
    ...
    React.useEffect(() => {
        axios.get(`https://newsapi.org/v2/everything?qInTitle=${topic}&language=en&sortBy=publishedAt&apiKey=eb634d3d41ca4a48b7379ba3936bc1f2`)
            .then(res => {
                setNews(res.data.articles)
                setNumberOfPages(res.data.totalResults)
                setLoading(false)
    ...
    return (
        <>
            ...
            <h1 className='text-sm font-mono my-2 text-center transition-all duration-75 ease-in  rounded-lg hover:bg-gray-100 p-10 mx-20'>Total Results: {numberOfPages}</h1>
        </>
    )
}

export default SearchedNewsSec